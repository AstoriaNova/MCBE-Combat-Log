...
export default class Combat {
  ...
  public static AfterOnHurt(event: EntityHurtAfterEvent) {
    const {
  ...
  public static async ItemUse(event: ItemUseAfterEvent) {
    const { source, itemStack } = event;

    if (!(source instanceof Player)) return;

    const adminTag = (world.getDynamicProperty("CombatLogAdminTag") as string) || Config.AdminTag;
    const adminItem = (world.getDynamicProperty("CombatLogAdminItem") as string) || Config.admin_item_typeId;

    if (!source.hasTag(adminTag)) return;
    if (itemStack.typeId !== adminItem) return;

    await this.AdminConfigMenu(source);
  }

  public static OnLeave(event: PlayerLeaveBeforeEvent) {
    const { player } = event;
    if (!player) return;
  ...
  public static OnSpawn(event: PlayerSpawnAfterEvent) {
    const { player } = event;
    if (!player) return;
    const playerId = player.id;
    ...
    if (logged) {
      ...
      player.sendMessage(
        "§cYou logged out during combat! Your inventory has been cleared as a penalty.",
      );
      player.setDynamicProperty(`combat_logged`, false);
    }
    if (Cache.CombatTime[playerId]) delete Cache.CombatTime[playerId];
  }

  private static async AdminConfigMenu(player: Player): Promise<void> {
    let currentAdminItem = world.getDynamicProperty("CombatLogAdminItem") as string || Config.admin_item_typeId;
    let currentCombatTimer = world.getDynamicProperty("CombatLogTimer") as number || Config.combat_time;
    let currentBlacklistedItems = world.getDynamicProperty("CombatLogBlackListedItems") as string || Config.BlackListeditems.join(",");
    let currentAdminTag = world.getDynamicProperty("CombatLogAdminTag") as string || Config.AdminTag;
    ...
    const confirmed = formValues[5] as boolean;
    if (!confirmed) {
      player.sendMessage("§cChanges were not confirmed! No changes saved.");
      return;
    }

    const newAdminItem = (formValues[1] as string).trim();
    const newCombatTimerStr = formValues[2] as string;
    const newCombatTimer = parseInt(newCombatTimerStr);
    const newBlacklistedItems = (formValues[3] as string).trim();
    const newAdminTag = (formValues[4] as string).trim();
  ...
  private static isBlacklistedItem(item: any): boolean {
    if (!item) return false;

    const blacklistedStr = world.getDynamicProperty("CombatLogBlackListedItems") as string || Config.BlackListeditems.join(",");
  ...
  private static DropInventory(player: Player) {
    const inventoryComp = player.getComponent("minecraft:inventory")!.container;
    const equippable = player.getComponent("minecraft:equippable");
    if (!inventoryComp) return;
    ...
    const invItems = Array.from({ length: inventoryComp.size }, (_, i) =>
      inventoryComp.getItem(i),
    );
